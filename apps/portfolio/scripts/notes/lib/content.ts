...
function readNotes(dir: string): Record<string, string> {
  return Object.fromEntries(readdirSync(dir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name)).flatMap(entry => {
    if (entry.name.startsWith('.') || ['Attachments', 'space', 'space 1', 'Tags'].includes(entry.name)) return []
    const file = path.join(dir, entry.name)
    if (entry.isDirectory()) return Object.entries(readNotes(file))
    return entry.name.endsWith('.md') ? [[path.relative(contentRoot, file), readFileSync(file, 'utf8')]] : []
  }))
}
const manifest = { files: readNotes(contentRoot) }
...
/* Manifest keys (and NoteData.filePath) are the POSIX path relative to
   content/, matching what path.relative(CONTENT_DIR, filePath) produced. */
function fileToSlug(relative: string): string {
...
export function getAllNotes(): NoteData[] {
  ...
  for (const [filePath, raw] of Object.entries(manifest.files)) {
