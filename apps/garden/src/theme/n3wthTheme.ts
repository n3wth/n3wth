...
export const n3wthTheme = defineTheme({
  ...
  typography: {
    ...
    // Satoshi is the display voice (matches n3wth.com). It has to be set
    // here, not overridden in globals.css: the generated theme redeclares
    // --font-family-heading on a scope closer to the headings than :root,
    // so the override never reached them and the two Satoshi files shipped
    // on every page unused.
    heading: {
      family: 'Satoshi',
      fallbacks: "'Geist Sans', system-ui, -apple-system, sans-serif",
    },
    code: {
      family: 'Geist Mono',
