...
/* Notes come from the committed content-manifest.json (regenerated by
   scripts/content-manifest.mjs on every build), not the filesystem:
   the Cloudflare workerd preview has no fs at runtime, so readdirSync/
   readFileSync here threw 500s on every re-rendered route. */
