...
Run installation at the repository root. The root lockfile owns dependencies for
applications and the shared library. Do not fix a workspace resolution problem by
creating a lockfile local to one app or installing another copy of UI.
...
`publish-ui.yml` on a `v*` tag. Bump the version and add a changelog entry
for library changes that consumers see; do not run `npm publish` locally.
Changes to documentation only do not need a package release.
