...
Run these checks that only read from the consuming application:
...
The release package check uses `scripts/pack-ui.mjs` to remove excluded Suisse font rules from the staged CSS. Shipped font URLs use paths relative to the package. External consumers use the theme's fallback to the system font. The release workflow publishes the exact tarball that passes the consumer checks.
...
`useTheme` uses `defaultTheme` during server rendering, but reads local storage and system preferences in the browser. Avoid assuming those initial values match. Keep any initialization that happens before paint aligned with the hook's storage key and `data-theme` attribute. The current UI docs app also sets `data-astryx-theme="n3wth"` before React starts.

A script that runs before paint addresses initial document colors. Text or icons that depend on the theme and render on the server still need a consistent rendering strategy in the application. Test a saved light preference, no saved preference, and disabled storage.
...
3. Push a tag `v<version>` on the merge commit, matching the version in `packages/ui/package.json`. This triggers **Publish**.
...
The workflow can also be dispatched manually on `main`. Publishing requires npm trusted publisher configuration for owner `n3wth`, repository `n3wth`, and workflow `publish-ui.yml`. The publish script skips a version that is already on npm and fails on unexpected registry lookup errors.
...
- [Publish workflow](https://github.com/n3wth/n3wth/blob/main/.github/workflows/publish-ui.yml)
