---
title: "Troubleshooting and releases"
description: "Diagnose import, CSS, font, and theme issues, and understand the @n3wth/ui 2.x release workflow."
---
...
The source theme selects Suisse Intl, and `site.css` references `@n3wth/ui/fonts/SuisseIntl-*.woff2`. The npm `files` allowlist deliberately excludes those commercial font binaries. This differs from a source workspace where the files exist.

The font README describes system-font fallback as intended for external consumers. That fallback can occur only if CSS reaches the browser: bundlers that resolve font URLs during compilation may fail first. A successful workspace build therefore does not prove that a clean registry consumer can resolve those URLs.

If the error names a Suisse font:
...
3. Have the package maintainer correct the distributed CSS or validate an application-specific asset configuration before treating the external installation as supported.
...
1. Run `npm run changeset` at the repository root, select UI, and describe the consumer-facing change.
2. Merge the validated change. A successful, current push run of **Site CI** on `main` triggers **Release UI**.
3. Review the generated release PR. The version script updates package versions, changelog, the packaged starter's UI dependency, and the root lockfile.
4. Merge the release PR after validation. With no pending Changesets and `UI_NPM_PUBLISH_ENABLED=true`, the publish job checks UI, validates a packed consumer, and publishes that exact tarball with provenance.
...
| `npm run release:version` | Apply Changesets and update starter and lockfile; used by the release workflow |
...
- [Version script](https://github.com/n3wth/n3wth/blob/main/scripts/version-ui.mjs) and [publish script](https://github.com/n3wth/n3wth/blob/main/scripts/publish-ui.mjs)
