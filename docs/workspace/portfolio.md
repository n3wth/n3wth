...
- Required local command: npm run check (ESLint, Vitest and production build). The build reads committed content snapshots in `src/data/*.json` (GitHub stats, garden notes/index, `@n3wth/ui` version) and validates them offline; it does not fetch them. Refreshing those snapshots from their live sources is a separate command, `npm run content:refresh` (`apps/portfolio/scripts/refresh-content.mjs`), never run as part of a build. Build also prerenders routes from that source tree.
