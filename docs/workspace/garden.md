...
Validation: clean root npm ci succeeded; two history regression tests, typecheck and production webpack build passed (959 generated pages). Source checksum comparison covered 288 content/public/theme/history files with zero differences. Generic affected selection includes site-config before Garden. Browser and deployed serverless validation remain required before cutover.
