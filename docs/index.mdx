...
<Callout icon="sparkles" color="#8B5CF6">
  This hub documents the public contracts of the workspace. Each guide links to
  the source of truth so you can verify examples before shipping them.
</Callout>
...
**UI** is a dependency. Upgrade the package to receive shared component
changes. Use it when you want the same visual foundation across applications.

<Tip>
  If you are unsure where to start, use UI for an existing React application
  and Skills when the deliverable is an assistant workflow rather than runtime
  code.
</Tip>
