...
export function createSite(root, slug, title = slug) {
  ...
  const files = {
    ...
    'vercel.json': JSON.stringify({ git: { deploymentEnabled: true }, installCommand: `node ../../scripts/vercel-install.mjs @n3wth/${slug}`, buildCommand: `cd ../.. && npm run build -- --workspace @n3wth/${slug} --cache-ui`, outputDirectory: 'dist' }, null, 2) + '\n',
